...
bool isQueueFull()  { return rearAntrian == MAX - 1; }
bool isQueueEmpty() { return frontAntrian == -1 || frontAntrian > rearAntrian; }
bool isStackFull()  { return topRiwayat >= MAX - 1; }
bool isStackEmpty() { return topRiwayat < 0; }

void pushRiwayat(Penumpang* p) {
    if (isStackFull()) { cout << "Stack overflow! Riwayat penuh.\n"; return; }
    topRiwayat++;
    *(stackRiwayat + topRiwayat) = *p;
    cout << "Transaksi atas nama " << p->nama << " dicatat ke riwayat.\n";
}

void popRiwayat() {
    if (isStackEmpty()) { cout << "Stack underflow! Riwayat kosong.\n"; return; }
    Penumpang* p = (stackRiwayat + topRiwayat);
    cout << "Transaksi dibatalkan: " << p->nama << " | " << p->namaKereta << " | " << p->rute << "\n";
    topRiwayat--;
}

void peekRiwayat() {
    if (isStackEmpty()) { cout << "Riwayat kosong.\n"; return; }
    Penumpang* p = (stackRiwayat + topRiwayat);
...
void tampilRiwayat() {
    ...
    for (int i = topRiwayat; i >= 0; i--) {
        Penumpang* p = (stackRiwayat + i);
        cout << "[" << (topRiwayat - i + 1) << "] " << p->nama << " | " << p->namaKereta << " | " << p->rute << "\n";
    }
}

void enqueue(Penumpang* p) {
    if (isQueueFull()) { cout << "Queue overflow! Antrian penuh.\n"; return; }
    if (frontAntrian == -1) frontAntrian = 0;
    rearAntrian++;
    *(queueAntrian + rearAntrian) = *p;
    cout << p->nama << " berhasil masuk antrian.\n";
}

void dequeue() {
    if (isQueueEmpty()) { cout << "Queue underflow! Antrian kosong.\n"; return; }
    Penumpang* p = (queueAntrian + frontAntrian);
    cout << "\n=== TIKET DIPROSES ===\n";
    cout << "Nama   : " << p->nama << "\n";
    cout << "Kereta : " << p->namaKereta << "\n";
    cout << "Rute   : " << p->rute << "\n";
    pushRiwayat(p);
    frontAntrian++;
    if (isQueueEmpty()) frontAntrian = rearAntrian = -1;
}

void peekAntrian() {
    if (isQueueEmpty()) { cout << "Antrian kosong.\n"; return; }
    Penumpang* p = (queueAntrian + frontAntrian);
...
void tampilAntrian() {
    ...
    for (int i = frontAntrian; i <= rearAntrian; i++) {
        Penumpang* p = (queueAntrian + i);
        cout << "[" << (i - frontAntrian + 1) << "] " << p->nama << " | " << p->namaKereta << " | " << p->rute << "\n";
    }
}
